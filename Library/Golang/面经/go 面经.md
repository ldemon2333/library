...
1. **高吞吐量**：尽可能使用可用带宽，高效传输数据。
    
2. **低延迟**：避免网络过载，减少排队、重传带来的延迟。
    
3. **公平性**：在共享链路上，不同连接应公平竞争带宽。
    
4. **稳定性**：网络状态波动时，能快速收敛到平稳状态，避免剧烈抖动。
...
1. **拥塞控制** 通过 **慢启动、拥塞避免、快速重传/恢复** 实现对丢包和延迟的自适应控制。
    
2. **算法演进** 解决了不同网络环境下的性能和公平性：从 Tahoe→Reno→NewReno→CUBIC→BBR。
    
3. **现代场景** 建议选用 **CUBIC**（Linux 默认）或 **BBR**（更低延迟高吞吐）以获得更好网络性能。
