...
![[Pasted image 20241012212655.png]]
...
![[Pasted image 20241012212742.png]]

SM（Streaming Multiprocessors）称作流式多处理器，是GPU中的一个计算单元，核心组件包括多个 CUDA 核心、共享内存、寄存器等。SM可以同时执行多个线程，通过并行计算提升性能。它管理着线程的调度、执行和数据共享。SM 包含很多为线程执行数学运算的 core，是英伟达 GPU 的核心，在 CUDA 中可以执行数百个线程、一个 block 上线程放在同一个 SM 上执行，一个 SM 有限的 Cache 制约了每个 block 的线程数量。

![[Pasted image 20241012212814.png]]
...
![[Pasted image 20241012213113.png]]

在 Fermi 架构中，通过 CUDA 来控制具体的指令执行，是最小的运算执行单元。所以对于现在的英伟达 GPU 架构来讲，流处理器（SP）的数量就是 CUDA Core 的数量。一个 SM 中包含了 2 组各 16 个 CUDA Core，每个 CUDA Core 包含了一个整数运算单元 ALU（Arthmetic Logit Unit）和一个浮点运算单元 FPU（Floating Point Unit）。

![[Pasted image 20241012213244.png]]
...
![[Pasted image 20241012213352.png]]
...
当一个线程块被调度到SM上，它会被划分成多个Warp。SM会同时执行一个或多个Warp的所有线程。
同一个Warp中的所有线程会执行相同的指令，但可以处理不同的数据。如果 Warp 中的某些线程因条件判断或内存访问而需要等待，其他线程仍然能够继续执行。这种特性称为**动态调度**，提高了资源利用率。GPU 通过同时调度多个 Warp 来实现高并行度。多个 SM 可以同时处理不同的 Warp，从而提高计算效率。
...
![[Pasted image 20241012213545.png]]
...
![[Pasted image 20241012213613.png]]
...
![[Pasted image 20241012213634.png]]
...
![[Pasted image 20241012213732.png]]
...
![[Pasted image 20241012214314.png]]
...
![[Pasted image 20241012214552.png]]
