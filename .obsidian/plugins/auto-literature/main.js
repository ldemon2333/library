...
// autoliter/patternRecognizer.ts
...
var patternRecognizer_default = PatternRecognizer;

// autoliter/spiders/spider.ts
var import_obsidian4 = require("obsidian");

// autoliter/spiders/arxivInfo.ts
var import_rss_parser = __toESM(require_rss_parser());
var import_obsidian = require("obsidian");
var ArxivInfo = class {
  constructor() {
    this.base_url = "http://export.arxiv.org/api/query";
    this.feedParser = new import_rss_parser.default();
  }
  async getInfoByArxivId(arxivId) {
    if (arxivId.startsWith("arXiv:")) {
      arxivId = arxivId.slice(6);
    }
    const params = `?id_list=${arxivId}`;
    try {
      const respnse = await (0, import_obsidian.requestUrl)(this.base_url + params);
      const data = await this.feedParser.parseString(respnse.text);
      assert(data.items.length > 0, "ArxivId returns no result");
      assert(
        data.items.length === 1,
        "ArxivId should return only one result"
      );
      console.log(data);
      return this.extractInfo(data.items[0], arxivId);
    } catch (error) {
      throw new Error(
        `Error in getInfoByArxivId: ${error.message}(request url: ${this.base_url + params})`
      );
    }
  }
  extractInfo(data, arxivId) {
    const title = data["title"];
    const author = data["author"] || "No author";
    const journal = "Arxiv";
    ...
    return { title, author, journal, pubDate, url, pdfUrl };
  }
};
var arxivInfo_default = ArxivInfo;

// autoliter/spiders/crossrefInfo.ts
var import_obsidian2 = require("obsidian");
var CrossrefInfo = class {
  constructor() {
  ...
  async getInfoByDoi(doi) {
    ...
    try {
      const response = await (0, import_obsidian2.requestUrl)(url).json;
  ...
  extractInfo(data) {
    ...
    const author = data["author"] && data["author"].length > 0 ? data["author"][0]["given"] : "No author";
    const journal = ((_a = data["short-container-title"]) == null ? void 0 : _a[0]) || ((_b = data["container-title"]) == null ? void 0 : _b[0]) || "No journal";
    const pubDate = data["published"]["date-parts"][0].map((i) => String(i)).join("-");
    const url = data["URL"];
    return { title, author, journal, pubDate, url };
  }
};
var crossrefInfo_default = CrossrefInfo;

// autoliter/spiders/bmInfo.ts
...
var BMInfo = class {
  ...
  async getInfoByBMId(bmId) {
    const urls = this.server.map((server) => `${this.base_url}${server}/${bmId}`);
    for (let url of urls) {
      try {
        const response = await (0, import_obsidian3.requestUrl)(url).json;
        if (response["collection"].length > 0) {
          const data = response["collection"][0];
          if (data["published"] !== "NA") {
            return new crossrefInfo_default().getInfoByDoi(data["published"]);
          }
  ...
  extractInfo(data) {
    const title = data["title"];
    const author = data["author_corresponding"] || "No author";
...
var bmInfo_default = BMInfo;

// autoliter/spiders/spider.ts
var Spider = class {
  constructor() {
    this.arxivInfo = new arxivInfo_default();
    this.crossrefInfo = new crossrefInfo_default();
    this.bmInfo = new bmInfo_default();
  }
  classify(identifier) {
    if (identifier.match(/10\.(?!1101)[0-9]{4}\/.*/)) {
      ...
      /(arXiv:)?[0-9]{2}[0-1][0-9]\.[0-9]{3,}.*|.*\/[0-9]{2}[0-1][0-9]{4}/
    )) {
      return "arxivId";
    } else {
      return "unrecognized";
...
// autoliter/utils.ts
async function getReplaceDict(m, outputFormat) {
  let replaceDict = {};
  let pdfUrls = [];
  let paperInfo = [];
  const spider = new spider_default();
  const paperIDs = m.map(
    (literature) => {
      var _a;
      return ((_a = literature[0].split("{").pop()) == null ? void 0 : _a.split("}")[0]) || "";
    }
  );
  const results = await Promise.all(
    paperIDs.map(async (paperID) => {
      try {
        let paperInfo2 = await spider.getPaperInfo(paperID);
        paperInfo2.id = paperID;
        return paperInfo2;
      } catch (error) {
        return {
          id: paperID,
  ...
  results.forEach((result, index) => {
    ...
    if (result.error) {
      ...
      replaceDict[origin_string] = outputFormat.replace("${title}", title).replace("${author}", author).replace("${journal}", journal).replace("${pubDate}", pubDate).replace("${url}", url);
      pdfUrls.push(result.pdfUrl || "");
...
// autoliter/settings.ts
var import_obsidian5 = require("obsidian");
var AutoLiterSettingTab = class extends import_obsidian5.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian5.Setting(containerEl).setName("Regular expression").setDesc("regular expression to match the literature").addText(
      (text) => text.setPlaceholder("regular expression").setValue(this.plugin.settings.regExp).onChange(async (value) => {
        this.plugin.settings.regExp = value;
        this.plugin.paperRecognizer.pattern = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian5.Setting(containerEl).setName("Output format").setDesc("output format after update the note").addTextArea(
      (text) => text.setPlaceholder("output format").setValue(this.plugin.settings.outputFormat).onChange(async (value) => {
        this.plugin.settings.outputFormat = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian5.Setting(containerEl).setName("Auto download PDF").setDesc("Automatically download PDFs when updating notes").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.autoDownloadPDF).onChange(async (value) => {
        this.plugin.settings.autoDownloadPDF = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian5.Setting(containerEl).setName("PDF Storage Path Calculation").setDesc(
      "Determine how the path for storing downloaded PDF files is calculated, relative to the vault or the markdown file."
    ).addDropdown(
      (dropdown) => dropdown.addOptions({
        vault: "vault",
        mdFile: "mdFile"
      }).setValue(this.plugin.settings.pdfDownloadPathCalculation).onChange(async (value) => {
        this.plugin.settings.pdfDownloadPathCalculation = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian5.Setting(containerEl).setName("PDF Storage Path").setDesc("The path for storing downloaded PDF files").addText(
      (text) => text.setPlaceholder("path").setValue(this.plugin.settings.pdfDownloadPath).onChange(async (value) => {
...
var AutoLiter = class extends import_obsidian6.Plugin {
  async onload() {
    await this.loadSettings();
    this.addSettingTab(new AutoLiterSettingTab(this.app, this));
    this.paperRecognizer = new patternRecognizer_default(`${this.settings.regExp}`);
    const ribbonIconEl = this.addRibbonIcon(
      "book-down",
      "AutoLiter",
      async (evt) => {
        new import_obsidian6.Notice("start updating vault.");
        const activeFile = this.app.workspace.getActiveFile();
        if (activeFile) {
          await this.updateFile(activeFile, this.paperRecognizer);
        }
        new import_obsidian6.Notice("finish updating vault.");
      }
    );
    this.addCommand({
      id: "update-vault",
      ...
      editorCallback: async (editor) => {
        new import_obsidian6.Notice("start updating vault.");
        await Promise.all(
          this.app.vault.getMarkdownFiles().map(
            (file) => this.updateFile(file, this.paperRecognizer)
          )
        );
        new import_obsidian6.Notice("finish updating vault.");
      }
    ...
    this.addCommand({
      ...
      editorCallback: async (editor) => {
        new import_obsidian6.Notice("start updating selection.");
        const selection = editor.getSelection();
        editor.replaceSelection(
          await this.updateSelected(selection, this.paperRecognizer)
        );
        new import_obsidian6.Notice("finish updating selection.");
      }
    });
  }
  onunload() {
  }
  async updateFile(file, paperRecognizer) {
    const content = await this.app.vault.read(file);
    const m = paperRecognizer.findAll(content);
    console.log(this.app.workspace.getActiveFile());
    if (m && m.length != 0) {
      const progressNotice = new import_obsidian6.Notice(`Updating ${file.path}...`);
      ...
      this.app.vault.process(file, (data) => {
        Object.keys(replaceDict).forEach((key, idx) => {
          ...
          try {
            this.settings.autoDownloadPDF && this.downloadPdf(
              pdfUrls[idx],
  ...
  async updateSelected(selection, paperRecognizer) {
    const m = paperRecognizer.findAll(selection);
    if (m && m.length != 0) {
      const progressNotice = new import_obsidian6.Notice(`Updating selection...`);
      const [replaceDict, pdfUrls, paperInfo] = await getReplaceDict(
        m,
        this.settings.outputFormat
      );
    ...
    try {
      ...
      try {
        if (this.settings.pdfDownloadPathCalculation === "mdFile") {
          const activeFile = this.app.workspace.getActiveFile();
          if (activeFile) {
            const activeFileFolder = activeFile.path.split("/").slice(0, -1).join("/");
      ...
      const filePath = this.joinPaths(folderPath, fileName);
      await vault.createBinary(filePath, uint8Array);
      new import_obsidian6.Notice(`Downloaded PDF: ${fileName}`);
    } catch (error) {
      throw new import_obsidian6.Notice(`Error downloading PDF: ${error.message}`);
    }
  }
  async loadSettings() {
